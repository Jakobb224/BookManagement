...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    if (!empty($autor) && !empty($standort) && !empty($title)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO backup (Autor, Standort, Title) VALUES (:autor, :standort, :title)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM backup WHERE Title = :title");
...
try {
    $sql = "SELECT * FROM backup ORDER BY Title ASC";
...
<body>
    ...
    <div class="center">
        <form id="add" method="POST">
            ...
            <button class="button" type="submit" name="add">Medium hinzufügen</button>
        </form>
    </div>
            ...
            <tbody>
                <?php foreach ($mediums as $medium): ?>
			     ...
			     <form method="POST">
                                 ...
                                 <button class="button" type="submit" onclick="return confirm('Möchtest du dieses Medium wirklich löschen?');">Löschen</button>
                             </form>
			</td>
		    </tr>
                <?php endforeach; ?>
